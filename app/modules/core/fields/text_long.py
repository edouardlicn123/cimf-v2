...
class TextLongField(BaseField):
    name = 'text_long'
    label = '带格式长文本'
    widget = 'rich_text_long'
    properties = ['value', 'format']
    
    def render(self, value, mode='edit'):
        if mode == 'view':
            return value.get('value', '')
        return f'<div class="rich-text-editor" data-field="{self.field_name}"><textarea name="{self.field_name}" class="form-control rich-text" rows="8">{value.get("value", "")}</textarea></div>'
