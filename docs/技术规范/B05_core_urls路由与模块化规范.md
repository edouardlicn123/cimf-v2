...
| 文件 | 路由数量 | 功能 |
|------|----------|------|
| `core/urls.py` | ~30 | 核心应用路由 |
| `core/node/urls.py` | ~15 | 节点系统路由 |
| `core/smtp/urls.py` | ~5 | 邮件系统路由 |
| `core/importexport/urls.py` | ~15 | 导入导出路由 |
| `modules/urls.py` | ~10 | 模块动态路由 |
...
urlpatterns = [
    ...
    # 导入导出路由
    path('importexport/', include('core.importexport.urls')),
...
| `importexport/` | importexport_dashboard | importexport_dashboard | 导入导出仪表盘 |
...
| `profile/` | profile_view | profile_view | 个人资料 |
| `profile/settings/` | profile_settings | profile_settings | 个人设置 |
| `profile/settings/homepage/` | homepage_settings | homepage_settings | 首页设置 |
| `user/nav-cards/` | navigation_settings | navigation_settings | 导航卡片设置 |
...
| `/profile/` | 个人中心 | `/profile/settings/`, `/profile/settings/homepage/` |
...
- [ ] 补充子模块路由清单（node, smtp, importexport）
- [ ] 添加路由测试规范
- [ ] 补充 API 版本管理建议
