...
功能说明：
    ...
    初始化阶段：
    【阶段1】数据库结构 - 执行 Django migrations
    【阶段2】系统配置 - 设置、权限、词汇表
    【阶段3】用户管理 - 创建管理员
    【阶段4】业务模块 - 扫描安装模块、注册节点类型
    【阶段5】样本数据 - 客户样本数据
...
def init_database(with_data: bool = False, force: bool = False, dry_run: bool = False,
    ...
    try:
        if not dry_run:
            ...
            for m in modules:
                if m.get('is_registered') and m.get('is_installed'):
                    skipped_count += 1
                    continue
            ...
            if installed_count > 0:
                msg += f"，已安装 {installed_count} 个"
            if skipped_count > 0:
                msg += f"，跳过 {skipped_count} 个已安装模块"
    ...
    # ===== 【阶段5】样本数据 =====
    print_section("阶段5：样本数据")
    
    print_step("5.1", "初始化海外客户样本")
    try:
        if not dry_run:
            from django.core.management import call_command
            from io import StringIO
            ...
            result = out.getvalue().strip().split('完成! ')[-1]
            print(colored(f"    ✓ 海外客户样本初始化完成: {result}", "green"))
        else:
            print(colored("    [模拟] 将调用 init_overseas_customers 命令", "yellow"))
    except Exception as e:
        print(colored(f"    ✗ 海外客户样本初始化失败: {str(e)}", "red"))

    print_step("5.2", "初始化国内客户样本")
    try:
        if not dry_run:
            from django.core.management import call_command
            from io import StringIO
            ...
            result = out.getvalue().strip().split('完成! ')[-1]
            print(colored(f"    ✓ 国内客户样本初始化完成: {result}", "green"))
        else:
            print(colored("    [模拟] 将调用 init_domestic_customers 命令", "yellow"))
    except Exception as e:
        print(colored(f"    ✗ 国内客户样本初始化失败: {str(e)}", "red"))
